...
const buildMetrics = async () => {
    ...
    const byCategory = categories.map((category) => {
        ...
        const valuesBySignal = new Map();
        series.forEach((entry) => {
            valuesBySignal.set(entry.signalKey, entry.points.map((point) => point.value));
        });
        ...
        const correlationVelocity = [];
        const clusterStability = [];
        ...
        weeks.forEach((week, index) => {
            ...
            const matrix = windowed.map(() => new Array(signalKeys.length).fill(0));
        ...
        const weekly = weeks.map((week, index) => {
            ...
            const syncEvent = varianceValue !== null &&
                velocityValue !== null &&
