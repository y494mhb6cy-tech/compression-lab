...
const baseValue = (signalKey, category) => {
    const profile = baseProfile[category];
    switch (signalKey) {
...
const noteForShock = (shock) => (Math.abs(shock) > 0.12 ? 'Macro shock period' : undefined);
const generate = async () => {
    ...
    categories.forEach((category) => {
        signalConfigs
            ...
            const source = sources[signal.key] ?? 'sample';
