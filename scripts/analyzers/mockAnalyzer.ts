...
const strengthLabel = (value: number | null) => {
...
const clusterLabel = (value: number | null) => {
  if (value === null || Number.isNaN(value)) return 'insufficient data';
  if (value > 0.8) return 'cluster memberships held steady';
  if (value > 0.6) return 'clusters shifted modestly';
  return 'clusters reshuffled noticeably';
};

export const analyze = ({ category, current, previous }: AnalyzerInput): AnalyzerOutput => {
  const prevVariance = previous?.varianceIndex ?? null;
  const prevCorrelation = previous?.avgAbsCorrelation ?? null;

  const varianceDelta =
    prevVariance !== null && current.varianceIndex !== null
      ? current.varianceIndex - prevVariance
      : null;
  const correlationDelta =
    prevCorrelation !== null && current.avgAbsCorrelation !== null
      ? current.avgAbsCorrelation - prevCorrelation
      : null;

  const interpretation =
    varianceDelta !== null && correlationDelta !== null
  ...
  const paragraph1 =
    `This week's structure readout for ${category} compares normalized postings, wage, and demand proxies ` +
    ...
    `Average absolute correlation is ${format(current.avgAbsCorrelation)} with velocity at ${format(
      current.correlationVelocity
    )}, ` +
    'capturing whether alignment across signals is accelerating or fading. ' +
    `Together this suggests ${interpretation}.`;

  const paragraph2 =
    `Cluster stability is ${format(current.clusterStability)}, meaning ${clusterLabel(
      current.clusterStability
    )}. ` +
  ...
  const paragraph3 =
    'These results are descriptive and conservative, built from weekly aggregates and simple normalization. ' +
