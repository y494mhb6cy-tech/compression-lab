...
export const kMeansClusters = (vectors: number[][], k: number, iterations = 12) => {
...
export const clusterStabilityScore = (prev: number[] | null, current: number[] | null) => {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = i + 1; j < n; j += 1) {
      ...
      if (samePrev === sameCurrent) stablePairs += 1;
      totalPairs += 1;
    }
  }
